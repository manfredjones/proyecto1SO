...
// Parsea un archivo .ani y llena la estructura Animation con los objetos leídos
int parse_animation(const char* filename, Animation* anim) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (strncmp(line, "CANVAS", 6) == 0) {
            ...
            // Guardar objeto anterior si había uno abierto
            if (in_object) {
            ...
            // Leer ID antes de limpiar estructura
            char temp_id[32];
            ...
            sscanf(line, "MOVE TO %d %d DURATION %d",
                   &current.movement.to.x, &current.movement.to.y,
                   &current.movement.duration);
...
// Dibuja el estado actual de los objetos en el canvas ASCII
...
// Actualiza la posición de todos los objetos según el tiempo actual
void update_objects(Animation* anim, int time) {
    for (int i = 0; i < anim->object_count; i++) {
        ...
        if (obj->movement.type == MOVE_LINEAR) {
            ...
            obj->current_pos.x = obj->position.x +
                                 (int)((obj->movement.to.x - obj->position.x) * progress);
            obj->current_pos.y = obj->position.y +
                                 (int)((obj->movement.to.y - obj->position.y) * progress);
            ...
            // Rebote horizontal
            if (obj->current_pos.x <= obj->movement.bounce.x1 ||
                obj->current_pos.x >= obj->movement.bounce.x2)
                obj->movement.bounce.dx *= -1;

            // Rebote vertical
            if (obj->current_pos.y <= obj->movement.bounce.y1 ||
                obj->current_pos.y >= obj->movement.bounce.y2)
                obj->movement.bounce.dy *= -1;
        }
    }
}

// Ejecuta la animación completa en modo secuencial (no paralelo)
void run_animation(Animation* anim) {
    ...
    while (time <= max_time) {
        ...
        usleep(100 * 1000);  // 100 ms por frame
        time += 100;
    }
}
